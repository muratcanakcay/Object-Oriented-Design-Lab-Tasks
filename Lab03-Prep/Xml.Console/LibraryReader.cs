...
namespace Xml.Console
{
    public static class LibraryReader
    {
        ...
        public static Library.goods ReadLibrary(string path)
        {
            if (!Validate(path)) return null;
            var serializer = new XmlSerializer(typeof(Library.goods));
            return (Library.goods)serializer.Deserialize(new FileStream(path, FileMode.Open, FileAccess.Read));
        }
        ...
        public static bool Validate(string path)
        {
            ...
            settings.Schemas.Add("http://example.org/mca/food-example", "food-example.xsd");
