...
namespace Xml.Console
{
    public static class LibraryReader
    {
        ...
        public static Library.formulaT ReadLibrary(string path)
        {
            if (!Validate(path)) return null;
            var serializer = new XmlSerializer(typeof(Library.formulaT));
            return (Library.formulaT)serializer.Deserialize(new FileStream(path, FileMode.Open, FileAccess.Read));
        }
        ...
        public static bool Validate(string path)
        {
            ...
            settings.Schemas.Add("http://example.org/mca/tortoise", "tortoise_s.xsd");
